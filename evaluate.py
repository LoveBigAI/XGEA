...
class evaluate:
    def __init__(self,dev_pair):
        ...
        def CSLS(tensor):
            ...
            val  = -tf.sort(-sim,axis=-1)
            # return [K.expand_dims(rank[:,0],axis=0),K.expand_dims(sim[:,rank[:,0]][:,0],axis=0)]
            return [K.expand_dims(rank[:,0],axis=0),K.expand_dims(val[:,0],axis=0)]
    ...
    def CSLS_cal(self, Lvec,Rvec,evaluate = True,batch_size = 1024):
        ...
        if evaluate:
            ...
            for epoch in range(len(Lvec) // batch_size + 1):
                ans_rank = np.array([i for i in range(epoch * batch_size,min((epoch+1) * batch_size,len(Lvec)))])
                r_rank_index,r_rank_val=self.rank_model.predict([R_sim[epoch],np.concatenate(LR,axis=1),RL[epoch],np.expand_dims(ans_rank,axis=0)])
                l_rank_index,l_rank_val=self.rank_model.predict([L_sim[epoch],np.concatenate(RL,axis=1),LR[epoch],np.expand_dims(ans_rank,axis=0)])
            ...
            return np.concatenate(r_rank,axis=1)[0],np.concatenate(l_rank,axis=1)[0] ,np.concatenate(r_rank_val_list,axis=1)[0],np.concatenate(l_rank_val_list,axis=1)[0] 
